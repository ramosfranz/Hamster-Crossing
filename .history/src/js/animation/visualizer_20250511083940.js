import { rows, cols, getGrid, getGoal, getStart } from '../manager/gridManager.js';
...
import { 
    ...
    redrawSeedOverPath,
    setGoalReached,
} from './seedAnimation.js';
import { getHamsterAssets } from '../themeManager.js';
...
export const hamsterSpriteSheet = new Image();
...
// Import the theme management functionality from themeManager.js
import { getHamsterAssets } from '../themeManager.js';
...
// Sprite sheets - will be updated based on selected hamster
export const spriteSheet = new Image();
export const hamsterSpriteSheet = new Image();

// Initialize with theme assets based on localStorage
function initializeThemeAssets() {
    // Get selected hamster from localStorage
    const selectedHamster = localStorage.getItem("selectedHamster") || "hamu";
    
    // Load assets based on the selected hamster
    loadHamsterAssets(selectedHamster);
    
    console.log(`Visualizer initialized with ${selectedHamster} theme`);
}

// Function to load assets for the selected hamster
function loadHamsterAssets(selectedHamsterKey) {
    // Get assets from themeManager.js
    const hamsterAssets = getHamsterAssets(selectedHamsterKey);
    
    // Update sprite sources
    spriteSheet.src = hamsterAssets.ground;
    hamsterSpriteSheet.src = hamsterAssets.sprite;
    
    // Apply background if canvas container exists
    const canvasContainer = document.querySelector('.canvas-container');
    if (canvasContainer) {
        canvasContainer.style.background = hamsterAssets.background;
    }
    
    console.log(`Loaded assets for ${selectedHamsterKey}:`, hamsterAssets);
}
...
export async function drawPathWithHead(ctx, cameFrom, current, lineColor, headColor) {
    ...
    const algorithmId = canvas.id || "unknown";
    
    // Reset goal reached for this algorithm
    setGoalReached(algorithmId, false);;
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        // If this is the goal, the hamster should remain visible
        // We won't redraw the seed animation here
        if (cell.x === goalPos.x && cell.y === goalPos.y) {
            // We've reached the goal - let's make sure the hamster stays visible
            // by not calling redrawSeedOverPath() here
            setGoalReached(algorithmId, true);
            
            // If you want to draw a ground sprite under the hamster, do it here:
            const grid = getGrid();
            ...
            // We can also pause the animation if desired at this point
            // but we won't do that since you mentioned not wanting to stop the animation
        }
