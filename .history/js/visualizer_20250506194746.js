...
// Animation state
let isRunningPathAnimation = false;
...
export function drawGrid() {
    // Skip redraw if path animation is in progress
    if (isRunningPathAnimation) {
        console.log("Path animation in progress, skipping normal grid redraw");
        return;
    }
    ...
    for (const algorithmId in ctx) {
        ...
        if (seedImagesLoaded) {
            ...
            console.log("Seed images not loaded, attempting to load now");
...
export async function drawPathWithHead(ctx, cameFrom, current, lineColor, headColor) {
    // Pause seed animation while path is being drawn
    const wasSeedAnimating = pauseSeedAnimation();
    isRunningPathAnimation = true;
    ...
    try {
        ...
        // Draw the goal after completing the path
        if (seedImagesLoaded) {
            drawSeedGoal(ctx, goalPos, cellSize);
        }
    } finally {
        // Resume seed animation if it was running before
        isRunningPathAnimation = false;
        if (wasSeedAnimating) {
