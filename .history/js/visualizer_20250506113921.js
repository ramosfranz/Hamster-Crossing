...
// Modified loadSeedImages function
function loadSeedImages() {
    ...
    // Load each image and only start animation when all are loaded
    let loadedCount = 0;
...
// Modified startSeedAnimation function
export function startSeedAnimation() {
    ...
    // Ensure all seed images are loaded
    if (!seedImages.every(img => img.complete)) {
        return;
    }
    ...
    seedAnimationInterval = setInterval(() => {
        ...
        drawGrid(); // Redraw grid to update seed animation
...
function drawSeedGoal(ctx, goal, cellSize) {
    ...
    // Draw the seed centered in the cell
    ctx.drawImage(
        seedImage,
