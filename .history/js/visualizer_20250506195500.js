...
export async function drawPathWithHead(ctx, cameFrom, current, lineColor, headColor) {
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        // If this is the goal position, redraw the seed on top
        if (cell.x === goalPos.x && cell.y === goalPos.y && i < path.length - 1) {
            // Draw the seed first, then the hamster will be drawn over it
            redrawSeedOverPath();
        }
        ...
        // If this is not the last cell in the path, 
        // redraw previous cells to maintain the path
        ...
        // If this was the goal cell, redraw the seed after the hamster passes
        if (cell.x === goalPos.x && cell.y === goalPos.y) {
            redrawSeedOverPath();
        }
    }
    
    // Final step: Ensure seed is visible at the end
    if (seedImagesLoaded) {
