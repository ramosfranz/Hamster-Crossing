...
export function drawGrid() {
    ...
    for (const algorithmId in ctx) {
        ...
        if (hamsterSpriteSheet.complete) {
            ...
            currentCtx.drawImage(
                hamsterSpriteSheet,
                0, 0, HAMSTER_SPRITE_WIDTH, HAMSTER_SPRITE_HEIGHT, // Default front-facing frame
...
function animateSeed() {
    if (!seedVisible || seedImages.length === 0) return;
    ...
    ctxA.clearRect(goalX, goalY, cellSize, cellSize);
    ctxB.clearRect(goalX, goalY, cellSize, cellSize);

    ctxA.drawImage(spriteSheet, grassSx, grassSy, spriteWidth, spriteHeight, goalX, goalY, cellSize, cellSize);
    ctxB.drawImage(spriteSheet, grassSx, grassSy, spriteWidth, spriteHeight, goalX, goalY, cellSize, cellSize);

    ctxA.drawImage(seedImage, goalX + (cellSize - scaledWidth) / 2, goalY + (cellSize - scaledHeight) / 2, scaledWidth, scaledHeight);
    ctxB.drawImage(seedImage, goalX + (cellSize - scaledWidth) / 2, goalY + (cellSize - scaledHeight) / 2, scaledWidth, scaledHeight);
}
