...
export async function drawPathWithHead(ctx, cameFrom, current, lineColor, headColor) {
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        if (hamsterSpriteSheet.complete) {
            ...
            const [spriteRow, spriteCol] = SPRITE_MAP.front.transition;

            ctx.drawImage(
                hamsterSpriteSheet,
                spriteCol * HAMSTER_SPRITE_WIDTH,
                spriteRow * HAMSTER_SPRITE_HEIGHT,
                HAMSTER_SPRITE_WIDTH,
                HAMSTER_SPRITE_HEIGHT,
                goalPos.x * cellSize + (cellSize - scaledWidth) / 2,
                goalPos.y * cellSize + (cellSize - scaledHeight) / 2,
                scaledWidth,
                scaledHeight
            );
        }
