    ...
    seedFiles.forEach(file => {
        ...
        img.onload = () => {
            ...
            if (loadedCount === seedFiles.length) {
                startSeedAnimation();
                drawGrid(); // <- Add this line to ensure seed draws right after load
            }
        };
        img.src = file;
        seedImages.push(img);
...
function drawSeedGoal(ctx, goal, cellSize) {
    if (!seedImages.length || !seedImages[0].complete) return;

    // First draw the ground tile
    const groundSx = GROUND_SPRITE_WIDTH; // Center grass tile
    const groundSy = GROUND_SPRITE_HEIGHT;
    ctx.drawImage(
        spriteSheet,
        groundSx, groundSy, GROUND_SPRITE_WIDTH, GROUND_SPRITE_HEIGHT,
        goal.x * cellSize, goal.y * cellSize, cellSize, cellSize
    );

    // Then draw seed on top
...
export function startSeedAnimation() {
    ...
    if (!seedImages.every(img => img.complete)) return;
...
export function drawGrid() {
    for (const algorithmId in ctx) {
        ...
        // Debug: Draw a red rectangle at goal position
