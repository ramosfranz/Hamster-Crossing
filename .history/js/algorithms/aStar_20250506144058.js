...
import { drawPathWithHead, drawVisited } from '../visualizer.js';
...
export async function animateAStar(ctx, timerElement) {
    ...
    while (openSet.length > 0) {
        ...
        if (current.x === goal.x && current.y === goal.y) {
            ...
            await drawVisited(ctx, visitedOrder)
    ...
    await drawVisited(ctx, visitedOrder); // Still draw visited if path not found
