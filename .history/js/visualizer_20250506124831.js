...
export let searchOverlays = {};
export let pathOverlays = {};

export function clearOverlays() {
    for (const algorithmId in ctx) {
        searchOverlays[algorithmId] = [];
        pathOverlays[algorithmId] = [];
    }
    drawGrid();
}
...
function drawSeedGoal(ctx, goal, cellSize) {
    ...
    console.log('Drawing seed at:', goal, 'with cellSize:', cellSize);
    console.log('Seed images loaded:', seedImagesLoaded);
    console.log('Seed images available:', seedImages.length);
    
    if (!seedImagesLoaded || !seedImages.length) {
        console.warn('Seed images not ready');
        return;
    }
...
export function drawGrid() {
    ...
    for (const algorithmId in ctx) {
        ...
        if (searchOverlays[algorithmId]) {
            currentCtx.fillStyle = "rgba(200, 200, 255, 0.3)";
            searchOverlays[algorithmId].forEach(cell => {
                currentCtx.fillRect(cell.x * cellSize, cell.y * cellSize, cellSize, cellSize);
            });
        }

        // Draw path overlays (semi-transparent)
        if (pathOverlays[algorithmId]) {
            currentCtx.fillStyle = "rgba(255, 255, 0, 0.3)";
            pathOverlays[algorithmId].forEach(cell => {
                currentCtx.fillRect(cell.x * cellSize, cell.y * cellSize, cellSize, cellSize);
            });
        }
...
export function recordSearch(algorithmId, cell) {
    if (!searchOverlays[algorithmId]) {
        searchOverlays[algorithmId] = [];
    }
    if (!searchOverlays[algorithmId].some(c => c.x === cell.x && c.y === cell.y)) {
        searchOverlays[algorithmId].push({x: cell.x, y: cell.y});
    }
}
...
export async function drawPathWithHead(ctx, cameFrom, current, lineColor, headColor) {
    const path = [];
    const algorithmId = Object.keys(canvases).find(id => canvases[id] === ctx.canvas);
    const cellSize = ctx.canvas.width / cols;
    const goalPos = getGoal();
    ...
    pathOverlays[algorithmId] = [...path];

    let frameIndex = 0;
    for (let i = 0; i < path.length; i++) {
        const cell = path[i];
        
        // Redraw grid to maintain all elements
        drawGrid();
