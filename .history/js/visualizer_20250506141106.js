...
function drawFullPath(ctx, cameFrom, goalNode, startNode) {
    ctx.save();
    ctx.strokeStyle = 'orange';
    ctx.fillStyle   = 'red'; 
    ctx.lineWidth   = 3;
    ctx.globalCompositeOperation = 'source-over'; // draw on top:contentReference[oaicite:9]{index=9}
    
    // Draw line segments from start to goal
    ctx.beginPath();
    let node = goalNode;
    while (node !== startNode) {
        let parent = cameFrom.get(node);
        // Assume each node has (x,y) pixel coords or compute them:
    ...
    // Optionally draw a circle (head) at the goal
    ctx.beginPath();
    ctx.arc(goalNode.x, goalNode.y, 6, 0, 2*Math.PI);
    ctx.fill();
