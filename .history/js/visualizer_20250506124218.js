...
export async function drawPathWithHead(ctx, cameFrom, current, lineColor, headColor) {
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        if (cell.x === goalPos.x && cell.y === goalPos.y) {
            // When reaching goal, draw hamster on top of seed
            const direction = getMovementDirection(path, i, cell);
            const spriteFrame = getSpriteFrame(direction, i, frameIndex % ANIMATION_FRAMES);
            
            // Draw the path cell (semi-transparent overlay)
            ctx.fillStyle = lineColor;
            ctx.globalAlpha = 0.5; // Make path semi-transparent
        ...
        if (i > 0) {
            ...
            if (!(prevCell.x === goalPos.x && prevCell.y === goalPos.y)) {
                // Draw path overlay (semi-transparent)
                ctx.fillStyle = lineColor;
                ctx.globalAlpha = 0.5;
        ...
        // Draw current cell overlay (semi-transparent)
        ctx.fillStyle = lineColor;
        ctx.globalAlpha = 0.5;
        ctx.fillRect(cell.x * cellSize, cell.y * cellSize, cellSize, cellSize);
        ctx.globalAlpha = 1.0;
