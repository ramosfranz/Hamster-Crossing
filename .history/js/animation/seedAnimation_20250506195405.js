...
export function startSeedAnimation() {
    ...
    seedAnimationInterval = setInterval(() => {
        ...
        // Only redraw the seed at the goal position, not the entire grid
        for (const algorithmId in ctx) {
            const currentCtx = ctx[algorithmId];
            const canvas = canvases[algorithmId];
            const cellSize = Math.min(canvas.width / getGoal().cols, canvas.height / getGoal().rows);
            
            // Create a temporary canvas to avoid clearing existing drawings
            const tempCanvas = document.createElement('canvas');
            tempCanvas.width = cellSize;
            tempCanvas.height = cellSize;
            const tempCtx = tempCanvas.getContext('2d');
            
            // Draw seed on temporary canvas
            const goalPos = getGoal();
            drawSeedGoal(tempCtx, {x: 0, y: 0}, cellSize);
            
            // Draw seed on main canvas at goal position
            currentCtx.save();
...
// New function to ensure the seed is drawn over any path visualizations
